...
//            setting trampoline tiles
    ...
    public void set_totalTiles(int total_tiles){
        this.total_tiles = total_tiles;
    }

    void win() throws GameWinnerException{
    ...
    void save() throws IOException {
        ...
        try{
            savedGame = new ObjectOutputStream(new FileOutputStream((getPlayer_name() + "savedGame.txt")));
    ...
    void play() throws CricketBiteException, SnakeBiteException, VultureBiteException, TrampolineException, IOException {
        int roll = 1;
        ...
        while(location != total_tiles){
            ...
            if(prev < 0.25 * total_tiles && location >= 0.25 * total_tiles){
                Scanner s = new Scanner(System.in);
                System.out.println();
                System.out.println("First Check Point");
                System.out.println("Do you want to save the game? Enter yes or no");
                String saveOption = s.nextLine();
                if(saveOption.equals("yes")) {
                    save();
                    System.exit(1);
                }
            }
            else if(prev < 0.5 * total_tiles && location >= 0.5 * total_tiles){
                Scanner s = new Scanner(System.in);
                System.out.println();
            ...
            else if(prev < 0.75 * total_tiles && location >= 0.75 * total_tiles){
                Scanner s = new Scanner(System.in);
                System.out.println();
...
class Snake extends Type implements Serializable{
...
class Vulture extends Type implements Serializable{
...
class Cricket extends Type implements Serializable{
...
class White extends Type implements Serializable{
...
class Trampoline extends Type implements Serializable{
...
public class lab6 {
    ...
    public static Game load(String name) throws IOException, ClassNotFoundException{
        int flag = 0;
        Scanner s = new Scanner(System.in);
        Game game = new Game(10);
        ObjectInputStream loadGame = null;
        try{
            loadGame = new ObjectInputStream(new FileInputStream(name + "savedGame.txt"));
            game = (Game) loadGame.readObject();
            flag = 1;
            loadGame.close();
        }
        catch(FileNotFoundException e){
            System.out.println("No such player data exists.");
        }
        return game;
    }
    public static void main(String[] args) throws IOException, ClassNotFoundException{
        ...
        Scanner s = new Scanner(System.in);
        System.out.println("Do you want to load previously saved game? Enter yes or no");
        String saveOption = s.nextLine();
        Game game = new Game(10);
        if(saveOption.equals("yes")) {
        ...
        try{
            ...
            game.set_totalTiles(total_tiles);
